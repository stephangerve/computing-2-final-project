...
public class Tsunami extends Spell{
	...
	public Tsunami(){
		...
		setSpellDesc("Deal 2 damage to every monster on field except water element type monsters.");
	}
	public void activateSpell(Player your, Player opponent){
		for(int i = 0; i < your.field.count(); i++){
			if(your.field.get(i).cardType.getCardType() == "Monster"){
				if(((Monster) your.field.get(i)).elementType.getElementType() != "Water"){
		...
		for(int i = 0; i < opponent.field.count(); i++){
			if(opponent.field.get(i).cardType.getCardType() == "Monster"){
				if(((Monster) opponent.field.get(i)).elementType.getElementType() != "Water"){
